...
def get_data(uid):
    uid = str(uid)
    if uid not in data:
        data[uid] = {"balance": 10000, "wagered":0, "profit":0}
    return data[uid]
        ...
        async def cb(inter: discord.Interaction):
            ...
            if idx in self.mine_pos:
                for c in self.children:
                    if int(c.custom_id) in self.mine_pos:
                        c.label="BOMB"
                ...
                if len([c for c in view2.children if str(c.label).startswith("CASHOUT")])==0:
                    ...
                    async def cash_cb(i2: discord.Interaction):
                        if i2.user.id!=self.uid:
                            return await i2.response.send_message("Not yours",ephemeral=True)
                        await i2.response.defer()
                        win=int(self.bet*self.mult)
                        d=get_data(i2.user.id)
                        d["balance"]+=win
                        d["wagered"]+=self.bet
                        d["profit"]+=win-self.bet
                        await save_all()
                        embed2=discord.Embed(color=0x57F287, title="CASHOUT")
                        embed2.description = f"Won {fmt(win)} at {self.mult}x"
                        for c in view2.children:
                            c.disabled=True
                        await i2.edit_original_response(embed=embed2, view=view2)
                        view2.stop()
...
async def rocket_cmd(inter: discord.Interaction, bet: str):
    ...
    r = random.random()
    if r < 0.50:
        crash = round(random.uniform(1.0, 1.8), 2)
    elif r < 0.75:
        crash = round(random.uniform(1.8, 2.5), 2)
    elif r < 0.90:
        crash = round(random.uniform(2.5, 3.5), 2)
    elif r < 0.97:
        crash = round(random.uniform(3.5, 6.0), 2)
    else:
        crash = round(random.uniform(6.0, 100.0), 2)
    class RocketView(discord.ui.View):
        ...
        async def cash(self, i2: discord.Interaction, btn):
            ...
            if self.crashed:
                return
            await i2.response.defer()
            win=int(bval*self.cur)
            d2=get_data(i2.user.id)
            d2["balance"]+=win
            d2["profit"]+=win-bval
            d2["wagered"]+=bval
            await save_all()
            embed=discord.Embed(color=0x57F287, title="CASHOUT")
            embed.description = f"Cashed at {self.cur}x = {fmt(win)} | Crash was {crash}x"
            for c in self.children:
                c.disabled=True
            await i2.edit_original_response(embed=embed, view=self)
            self.stop()
    embed=discord.Embed(color=0x2B2D31, title="ROCKET INFINITE")
    embed.description = f"Bet {fmt(bval)} | Max infinite"
    view=RocketView()
    await inter.response.send_message(embed=embed, view=view)
    for i in range(1000):
        ...
        if view.cur >= crash:
            view.crashed=True
            embed2=discord.Embed(color=0xED4245, title="CRASHED!")
            embed2.description = f"Crashed at {crash}x | Lost {fmt(bval)}"
            ...
            for c in view.children:
                c.disabled=True
            try:
                await inter.edit_original_response(embed=embed2, view=view)
            except:
                pass
            view.stop()
            break
        else:
            embed.description = f"Bet {fmt(bval)} | Current {view.cur}x = {fmt(int(bval*view.cur))} | Max infinite"
            for c in view.children:
                c.label=f"CASHOUT {view.cur}x"
            try:
...
async def colordice_cmd(inter: discord.Interaction, bet: str, color: str):
    ...
    if color.lower() not in colors:
        return await inter.response.send_message(f"Pick {', '.join(colors)}", ephemeral=True)
    d["balance"]-=bval
    await save_all()
    ...
    if roll==color.lower():
        ...
        d["wagered"]+=bval
        await save_all()
        await inter.response.send_message(f"Rolled {roll} vs {color} | Lost {fmt(bval)}")

# CHICKEN CROSS - ROADS ONLY MULT
    ...
    def get_road_visual(self):
        ...
        for i in range(1, 7):
            ...
            if i < self.pos:
                ...
                roads.append(f"CHICKEN ROAD {i} | {mult}x | chicken here")
            elif i == self.pos + 1:
                roads.append(f"ROAD {i} | {mult}x | next")
            else:
                roads.append(f"ROAD {i} | {mult}x")
        roads.append("FINISH INFINITE")
        return "\n".join(roads)
...
async def chicken_cmd(inter: discord.Interaction, bet: str):
    ...
    embed=discord.Embed(color=0x2B2D31, title="Chicken Cross - Crossing Roads INFINITE")
    desc = f"Bet {fmt(bval)}\n\n{view.get_road_visual()}\n\nLane {view.pos} | {view.mult}x = {fmt(int(bval*view.mult))}\nMax infinite"
    embed.description = desc
    go_btn=discord.ui.Button(label=f"CROSS - Next {view.get_mult(1)}x", style=discord.ButtonStyle.primary)
    ...
    async def go_cb(i: discord.Interaction):
        ...
        if random.random()>safe:
            embed2=discord.Embed(color=0xED4245, title="HIT BY CAR! FRIED!")
            embed2.description = f"Bet {fmt(bval)}\n\n{view.get_road_visual()}\n\nFried at ROAD {view.pos+1}!\nLost {fmt(bval)}"
            d2=get_data(i.user.id)
            d2["wagered"]+=bval
            d2["profit"]-=bval
            await save_all()
            for c in view.children:
                c.disabled=True
            await i.edit_original_response(embed=embed2, view=view)
            view.stop()
        else:
            view.pos+=1
            view.mult=view.get_mult(view.pos)
            next_mult=view.get_mult(view.pos+1)
            cash_btn.disabled=False
            cash_btn.label=f"CASHOUT {view.mult}x = {fmt(int(bval*view.mult))}"
            go_btn.label=f"CROSS - Next {next_mult}x"
            embed2=discord.Embed(color=0x2B2D31, title=f"Chicken Cross - Road {view.pos} | {view.mult}x")
            embed2.description = f"Bet {fmt(bval)}\n\n{view.get_road_visual()}\n\nLane {view.pos} | {view.mult}x = {fmt(int(bval*view.mult))}\nNext {next_mult}x | Max INFINITE"
            await i.edit_original_response(embed=embed2, view=view)
    async def cash_cb(i: discord.Interaction):
        if i.user.id!=view.uid:
            return await i.response.send_message("Not yours",ephemeral=True)
        await i.response.defer()
        win=int(bval*view.mult)
        d2=get_data(i.user.id)
        d2["balance"]+=win
        d2["wagered"]+=bval
        d2["profit"]+=win-bval
        await save_all()
        embed2=discord.Embed(color=0x57F287, title="CASHOUT - Safe!")
        embed2.description = f"Lane {view.pos} | {view.mult}x = {fmt(win)}\nBalance {fmt(d2['balance'])}"
        for c in view.children:
            c.disabled=True
        await i.edit_original_response(embed=embed2, view=view)
        view.stop()
...
if not TOKEN:
    print("ERROR: Set TOKEN env var in Railway Variables")
else:
    bot.run(TOKEN)
